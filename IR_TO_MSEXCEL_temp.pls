...
as
  ...
  BEGIN
    ...
    if nvl(p_dynamic_action.attribute_03,'Y') = 'Y' then
      if v_affected_region_selector is not null then
       ...
       function getColumnsProperties(callback) {
               apex.server.plugin ( '''||v_plugin_id||''', 
                                  {x01: "G",
                                   x02: $v("pFlowId"),
                                   x03: $v("pFlowStepId")
                                  },                                   
    ...
    apex_javascript.add_library (p_name      => 'IR2MSEXCEL2',
  ...
  begin
    ...
    select column_id,
           ...
           case 
            when  data_type in ('DATE','TIMESTAMP_TZ','TIMESTAMP_LTZ','TIMESTAMP') then
                  apex_common.xml_to_xslx.convert_date_format_js(p_datatype => data_type, p_format => format_mask)
            else ''
           end date_format_mask_js,
           case 
            when  data_type in ('DATE','TIMESTAMP_TZ','TIMESTAMP_LTZ','TIMESTAMP') then
                  apex_common.xml_to_xslx.convert_date_format(p_format => nvl(format_mask,'DD.MM.YYYY'))
            else ''
    ...
    APEX_JSON.write('column_properties', l_cursor);
