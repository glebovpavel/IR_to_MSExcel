  ...
  begin
    if p_format is null then
      if p_datatype = 'TIMESTAMP_TZ' then
        v_str := ir_to_xml.get_current_format(dbms_types.TYPECODE_TIMESTAMP_TZ);
      elsif p_datatype = 'TIMESTAMP_LTZ' then
        v_str := ir_to_xml.get_current_format(dbms_types.TYPECODE_TIMESTAMP_LTZ);  
      elsif p_datatype = 'TIMESTAMP' then
        v_str := ir_to_xml.get_current_format(dbms_types.TYPECODE_TIMESTAMP);          
      elsif p_datatype = 'DATE' then
       v_str := ir_to_xml.get_current_format(dbms_types.TYPECODE_DATE);
      else
        v_str := ir_to_xml.get_current_format('');
      end if;
    else
      v_str := p_format;
    end if;
